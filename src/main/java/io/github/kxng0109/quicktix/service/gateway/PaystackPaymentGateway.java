...
public class PaystackPaymentGateway implements PaymentGateway {
	 ...
	 * @return a {@link GatewayInitializationResponse} containing the Paystack authorization URL,
	 * which the frontend will use to redirect the user and the ID of the transaction generated by Paystack.
	...
	public GatewayInitializationResponse initializePayment(Payment payment) {
		try {
			...
			Map<String, Object> body = Map.of(
					...
					"amount", amountInKobo,
					"currency", "NGN",
					"reference", payment.getIdempotencyKey(),
			...
			if (response != null && response.get("status").asBoolean()) {
				...
				// Return the authorization URL and the transaction ID.
				// The frontend will redirect the user to the authorization URL.
				return GatewayInitializationResponse.builder()
				                                    .clientSecret(
						                                    response.get("data").get("authorization_url").asText()
				                                    )
				                                    .transactionId(response.get("data").get("reference").asText())
				                                    .build();
			} else {
